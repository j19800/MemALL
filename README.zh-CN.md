...
<p align="center">
  <i>6 个 MCP 工具 · 11 层记忆生命周期 · 24 步自进化管线 · 知识图谱 · 多 Agent 共享记忆 · <b>Hook 驱动自动化</b></i>
</p>
...
3. **Hook 驱动自动化** — 保存记忆后自动触发轻量管线（分类+收敛+反思）；检索时自动注入待处理工作；管线完成时提供丰富报告
4. **L9 蒸馏** — 将原始对话压缩为结构化知识
5. **L11 领域智能** — 跨项目模式检测
6. **遗忘与自适应** — 基于 TTL 的衰减、低价值清理、自动重索引
7. **OODA 循环** — 观察 → 定向 → 决策 → 行动，无需人工干预
