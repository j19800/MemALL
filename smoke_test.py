...
try:
    ...
    if isinstance(tl, list):
        ...
        ok(f"timeline(7天) 返回类型 {type(tl).__name__}")
except Exception as e:
...
try:
    ...
    if edge_row:
        ...
        if isinstance(tr, dict):
            ...
            edges_count = tr.get("edges", [])
            ok(f"traverse(id={edge_row['source_id']}) nodes={len(nodes)} edges={len(edges_count)}")
        else:
...
try:
    ...
    if isinstance(vs, dict):
        ...
        ok(f"vector_search 返回 {type(vs).__name__}")
except Exception as e:
...
try:
    ...
    if extract_l6:
        ...
        ok("无 extract_step 产物（首次运行后应有数据）")
except Exception as e:
