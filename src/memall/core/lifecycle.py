"""
Lifecycle hook system — HookRegistry, dispatch_lifecycle, hook constants.

Moved from ``mcp/hooks.py`` to eliminate the ``core → mcp`` layer violation
(``core/thin_waist.py`` was importing from ``mcp/hooks.py``).

``mcp/hooks.py`` now re-exports everything from this module for backward
compatibility.
"""
