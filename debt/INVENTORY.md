...
|------|------|--------|------|----|----|----|-----|
| CORE | core/ | 7 | 2,465 | 2 | 7 | 4 | 2 |
| PL | pipeline/ | 40 | 11,528 | 5 | 12 | 6 | 4 |
| MCP | mcp/ | 12 | 4,193 | 3 | 4 | 3 | 1 |
| GW | gateway | 1 | ~2,500 | 3 | 3 | 2 | 1 |
| CLI | cli/ | 7 | 6,800 | 0 | 1 | 2 | 2 |
| TST | tests/ | ~30 | ~3,000 | 0 | 2 | 4 | 0 |
| SRH | search/ | 4 | 401 | 0 | 2 | 1 | 2 |
| GRP | graph/ | 5 | 753 | 0 | 1 | 1 | 1 |
| BRG | bridge/ | 7 | 631 | 0 | 1 | 1 | 0 |
| **合计** | | | **~28,057** | **13** | **33** | **24** | **13** |
...
- **发现时间：** 2026-06-26
- **描述：** `session_end()` 中 `l6_content` 拼接使用了未赋值的 `session_project` 变量，每次 >3 条记忆的 session 结束触发 `NameError`，被外层 `except Exception` 静默吞掉。
- **影响：** L6反思永远不会创建，且无报错提示。影响所有 4 个 agent 的 session 结束流程。
- **修复方式：** 从 `_harvest_session` 提取逻辑或将 `session_project` 从上游传递下来。
- **预估工时：** 15 分钟
- **责任方：** —
- **状态：** 待修复
...
- **影响：** 数据完整性——孤立 edge 指向不存在的 memory。
- **修复方式：** `try: ... finally: conn.execute("PRAGMA foreign_keys=ON")`。
- **预估工时：** 10 分钟
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** `_auth_middleware` 中 `if request.path.startswith("/api/"): return await handler(request)` 绕过所有 `/api/*` 端点的认证。文档写的是"要求 Bearer token"但实现矛盾。
- **影响：** 任何人可在端口上执行写操作（创建 discussion、respond、读取全部记忆）。
- **修复方式：** 删除该 bypass 或为 API 端点单独配置 scope token。
- **预估工时：** 30 分钟
- **责任方：** —
- **状态：** 待修复
...
- **影响：** 一次 POST 即可窃取网关主令牌，接管全部 API 访问。
- **修复方式：** 返回一次性配对码而非 master token，或要求 `/pair` 本身也需要预共享密钥。
- **预估工时：** 1 小时
- **责任方：** —
- **状态：** 待修复
...
- **影响：** 攻击者可用非数字 query param 使多个 handler 500。
- **修复方式：** 统一使用 `try: int(...) except: default` 或 Pydantic 模型校验。
- **预估工时：** 30 分钟
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** `/mcp` HTTP 端点（端口 9876）不检查 Bearer token、Origin、IP。任何人可调用 `tools/call` 执行任意 tool。
- **影响：** RCE 等效——攻击者可调用 `capture`、`smart_store`、`run_pipeline` 等所有 MCP 工具。
- **修复方式：** 添加 Bearer token 中间件，或绑定到 127.0.0.1（已部分实现但注释说"no dependency"暗示会放开）。
- **预估工时：** 1 小时
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** `str(uuid.uuid4())[:8]` 将 128 位 UUID 截断到 32 位空间。10 万次操作后碰撞概率约 50%。
- **影响：** Session ID 碰撞导致 session 数据污染。
- **修复方式：** 使用完整 uuid4 或自增 ID。
- **预估工时：** 5 分钟
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** 嵌套循环 `for i in range(len(rows)): for j in range(i+1, len(rows))` 遍历全部记忆计算 Jaccard。10,000 条 => 5,000 万次迭代，100,000 条 => 50 亿次。
- **影响：** 随记忆量增长 pipeline 执行时间非线性爆炸。
- **修复方式：** 加时间窗或类别分桶限制候选集大小（如 `WHERE created_at > ?`）。
- **预估工时：** 2 小时
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** 每行候选记忆额外执行 `COUNT(*) FROM edges`。每 batch 500 行 => 500 次额外查询。
- **影响：** classify 步骤随数据增长变慢。
- **修复方式：** 预聚合 `GROUP BY target_id` 一次查完。
- **预估工时：** 1 小时
- **责任方：** —
- **状态：** 待修复
...
- **发现时间：** 2026-06-26
- **描述：** 压缩模式嵌套 O(n²) Jaccard 对比，全表加载到 DataFrame。
- **影响：** 同 S0-008。
- **修复方式：** 同 S0-008 策略。
- **预估工时：** 2 小时
- **责任方：** —
...
- **发现时间：** 2026-06-26
- **描述：** `Memory` dataclass 缺少 `thread_id` 和 `agent_name_locked` 字段，但 DB schema 和 INSERT 语句都用了。`_row_to_memory()` 不返回这些字段，下游代码（如 context_assembler）无法通过 Memory 对象访问。
- **影响：** 代码可读性差，依赖方需要额外查 DB。
- **修复方式：** 补全 dataclass 字段 + 更新 `_row_to_memory`。
- **预估工时：** 15 分钟
- **责任方：** —
- **状态：** 待修复
...
- **描述：** `sentence_transformers` 缺失时 `_auto_embed` 被 `except Exception` 吞掉，每次 capture 向量化静默跳过。
- **影响：** 向量搜索无数据返回，用户以为是搜索逻辑问题。
- **修复方式：** 启动时检测依赖，配置开关明确关闭向量功能。
- **预估工时：** 30 分钟
- **责任方：** —
- **状态：** 待修复
...
```
总负债项：     83（13 S0 + 33 S1 + 24 S2 + 13 S3）
预估修复工时： ~60 小时（S0: 10h + S1: 15h + S2: 5h + S3: 30h）
S0 修复率：    0/13（0%）
S1 修复率：    0/33（0%）
上次负债扫描： 2026-06-26
```
