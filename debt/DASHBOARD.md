# 技术负债看板

> 自动更新时间：2026-06-26  
> 自动更新方式：`python debt/scan.py`  
> ⚠️ 行数数据已于 2026-06-26 经代码审计校准（之前 cli/ 和 tests/ 行数严重失真）

---

## 总体仪表盘

```mermaid
%%{init: {'theme': 'base', 'themeVariables': { 'pie1': '#ff4444', 'pie2': '#ff8800', 'pie3': '#ffcc00', 'pie4': '#4488ff'}}}%%
pie title 技术负债按等级分布（57 项）
    "S0 Critical" : 0
    "S1 Major" : 20
    "S2 Minor" : 24
    "S3 Architecture" : 13
```

| 指标 | 值 |
|------|-----|
| 总负债项 | **57** |
| 预估总工时 | **~30 小时** |
| 当前修复率 | **S0: 13/13 ✅ — S1: 17/37（45.9%）** |
| 代码总行数 | 32,813 |
| 负债密度 | 1.73 项/千行 |

---

## 模块热力图

\`\`\`
模块         S0  S1  S2  S3  总计  行数    密度(项/KLOC)
─────────────────────────────────────────────────────
pipeline/    0   11  6   4   21   11,518  1.82
mcp+gateway  0   2   5   2    9    6,885  1.31
core/        0   0   4   2    6    2,473  2.43
cli/         0   1   2   2    5    4,338  1.15
tests/       0   2   4   0    6   11,476  0.52
search/      0   2   1   2    5      401  12.47 🔴🔴
graph/       0   1   1   1    3      750  4.00
bridge/      0   1   1   0    2      631  3.17 ⚠️
─────────────────────────────────────────────────────
合计         0   20  24  13  57   38,472  1.48
\`\`\`

> ⚠️ **注意**：热力图仅覆盖 38,472/44,289 行（86.9%），缺失模块包括 lark/、api/、federation/、scheduler/、plugins/、migrations/ 等共 11 个模块约 5,800 行尚未纳入扫描。

---

## S0 Critical 修复看板

```
┌──────────┬──────────────────────────────────┬────────┬────────┬──────┐
│ ID       │ 问题                             │ 模块   │ 工时   │ 状态 │
├──────────┼──────────────────────────────────┼────────┼────────┼──────┤
│ S0-001   │ session_project NameError        │ PL     │ 15m    │ ✅   │
│ S0-002   │ PRAGMA foreign_keys=OFF          │ PL     │ 10m    │ ✅   │
│ S0-003   │ /api/* auth bypass               │ GW     │ 30m    │ ✅   │
│ S0-004   │ /pair token leak                 │ GW     │ 1h     │ ✅   │
│ S0-005   │ int() crash                      │ GW     │ 30m    │ ✅   │
│ S0-006   │ MCP HTTP no auth                 │ MCP    │ 1h     │ ✅   │
│ S0-007   │ UUID truncated to 32 bits        │ PL     │ 5m     │ ✅   │
│ S0-008   │ O(n²) link.py                    │ PL     │ 2h     │ ✅   │
│ S0-009   │ N+1 edge count                   │ PL     │ 1h     │ ✅   │
│ S0-010   │ enrich.py no LIMIT               │ PL     │ 1h     │ ✅   │
│ S0-011   │ O(n²) adaptive.py                │ PL     │ 2h     │ ✅   │
│ S0-012   │ Memory dataclass mismatch        │ CORE   │ 15m    │ ✅   │
│ S0-013   │ embedding silent fail            │ GRP    │ 30m    │ ✅   │
└──────────┴──────────────────────────────────┴────────┴────────┴──────┘
```

---

## 修复趋势追踪

```
日期        S0修复数   S1修复数   修复率    备注
──────────────────────────────────────────────
2026-06-26    0/13      0/37      0%       初始扫描
2026-06-26    13/13      0/37     100%       v0.1.11~v0.1.14 安全+性能+认证修复
2026-06-26    13/13      5/37     100%       Phase A: 配置原子写/回滚/动态日期/FD泄漏
2026-06-26    13/13     11/37     100%       Phase B: LOWER索引/N+1批查/forget LIMIT
2026-06-26    13/13     17/37     100%       Phase C: depth clamp/client_max_size/import路径安全 + 3 INVENTORY订正
```

---

## 负债年龄分布

```
┌─────┬──────────────────────────────┐
│ 🆕  │ 0-7 天（本次扫描发现）    │ 70 项
│ 📅  │ 8-30 天（上次扫描）       │ — 项
│ 🕰️  │ 30-90 天（遗留负债）      │ — 项
│ 🦖  │ 90+ 天（技术化石）        │ — 项
└─────┴──────────────────────────────┘
注：首次扫描全部标记为 🆕
```

---

## ✅ S0 已全部修复 / S1 批量进行中

```
S0 修复完成：13/13（100%）
S1 批量 Phase A+B+C：17/37（45.9%）— 2026-06-26
CORE 层 7/7 清零, MCP+GW 5/7 清零
覆盖范围：安全、数据、性能、静默失败、配置、事务、索引、N+1、
           FD泄漏、LOWER函数索引、depth clamp、client_max_size
修复版本：v0.1.11 ~ v0.1.14
下一步建议：继续 S1 剩余 20 项（PL 11 + MCP/GW 2 + CLI 1 + TST 2 + SRH 2 + GRP 1 + BRG 1）
```
---

## 负债排除清单（已确认不修）

| ID | 问题 | 排除理由 | 排除人 | 日期 |
|----|------|---------|--------|------|
| — | — | — | — | — |
